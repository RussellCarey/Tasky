...
export const sendWelcomeEmail = async (username: string, email: string, activateString: string) => {
  const msg = {
    ...
    text: `Welcome! ${username} to my Tasky task recording app!.. (Email design in progress). Please go to the following link to activate your account: http://localhost:3000/auth?=${activateString}.`,
    html: `<strong>Welcome! ${username} to Tasky.</strong>
    <br>
    <p>Email design in progress</p>
    <br>
    <p> Please click <a href="http://localhost:3000/auth?=${activateString}">here</a> to confirm and activate your account.`,
  };
