...
export const login = async (commandObject: ICommandInitalObject) => {
	...
	try {
		if (commandObject.args.length !== 2) return ['Please type login followed by only the username and password to use this function.'];
...
export const signup = async (commandObject: ICommandInitalObject) => {
	try {
		...
		if (commandObject.args.length > 4) return ['Please type signup followed by desired username, email, and passwords to use this function.'];

		// Run checks on inputs..
		if (!checkValidUsername(commandObject.args[0])) return ['Username should contain no numbers, spaces or special characters.'];
